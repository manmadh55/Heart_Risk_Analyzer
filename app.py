...
# Load reference background dataset for SHAP
@st.cache_data
def load_shap_bg(_scaler, _columns):  # underscore prefix tells Streamlit not to hash these args
    filename = "processed.cleveland.data"
    if not os.path.exists(filename):
        return None, None
    ...
    # FIX 1: Correct indentation — fillna().astype(int) applied inside the loop to every column
    for col in ['sex', 'cp', 'fbs', 'restecg', 'exang', 'slope', 'ca', 'thal']:
        df[col] = pd.to_numeric(df[col], errors="coerce")
        df[col] = df[col].fillna(0).astype(int)
    ...
    # FIX 2: Match training columns exactly using the loaded columns.pkl
    for col in _columns:
...
X_scaled_bg, encoded_cols = load_shap_bg(scaler, columns)
...
if submit_button:
    ...
    # FIX 4: Safety check — add any missing columns before reordering
    input_df = pd.DataFrame([raw_input])
    ...
    if X_scaled_bg is not None:
        with st.spinner("Generating SHAP waterfall plot..."):
            try:
                # FIX 5: Removed feature_perturbation="interventional" for SHAP compatibility
                explainer = shap.LinearExplainer(model, X_scaled_bg)
                shap_values = explainer(scaled_input)
